...
'''
Teleop program, based on a command line interface. This program will order the base,the upper and the arm of the paint robot.
...
TOPIC_ODOM = '/odom'   #base

DEFAULT_SPEED_LIN = 0.3
DEFAULT_SPEED_ANG = 0.4

# utilities functions
#如果是用sin()或cos()算出来的数，不是float，是double类型
def is_float(string):
...
# messages generation
#这个可以考虑修改成直接接收x和y的速度，不过需要看后面的parse函数，矩阵后面的两位感觉还算好用。
...
def get_duration_lin(speed_lin, length):
    ...
    return float(math.fabs(length / speed_lin))   #绝对值
...
def pub_base_planar(pub, msg, duration):
    ...
    msg_init = get_twist(0.0, 0.0, 0.0, 1.0)    #这里之后需要注意
...
class CommandYouBot(cmd.Cmd):             #主要的控制函数，改个名
